...
export default {
  ...
  mounted() {
    ...
    // get a document
    /*
    let docRef = this.$fire.firestore.collection("reportase_langsung").doc("tahun_baru_2023")
    docRef.get()
    .then((doc) => {
        if (doc.exists) {
          self.document = doc
          self.docId = doc.id
          self.documentData = doc.data()
          console.log("Document data:", doc.data());
        } else {
          // doc.data() will be undefined in this case
          self.document = "no such document"
    ...
    */
    ...
    // set a document
    // let newDocRef = this.$fire.firestore.collection("reportase_langsung").doc("tahun_baru_2023").collection("contents").doc("kedua")
    // newDocRef.update({
    //   published_date: new Date(),
    //   text: "tahun baru telah tiba lagi",
    //   type: "video",
    //   url: "url-video"
    // })
    // .then(() => {
    ...
    // get a collection
    this.docRef = this.$fire.firestore.collection("reportase_langsung").doc("tahun_baru_2023").collection("contents")
    this.docRef.orderBy("published_date").onSnapshot((querySnapshot) => {
        console.log("keganti")
        self.events = [];
        querySnapshot.forEach((doc) => {
            self.events.push(doc.data());
        });
